document.addEventListener("DOMContentLoaded", function () {
  ...
  function initTextTruncation() {
    ...
    console.log(`Found ${captions.length} post captions`);

    captions.forEach((caption, index) => {
      if (!caption) return;

      const fullText = caption.getAttribute('data-full-text') || caption.textContent.trim();
      console.log(`Caption ${index}: ${fullText.length} characters - "${fullText.substring(0, 50)}..."`);
      
      // Only truncate if text is longer than maxLength
      if (fullText.length > maxLength) {
        console.log(`Truncating caption ${index} (${fullText.length} chars)`);
        const truncatedText = fullText.substring(0, maxLength);
        
        // Check if already processed
        if (caption.querySelector('.see-more-btn')) {
          console.log(`Caption ${index} already processed`);
          return;
        }
